...
function setActiveStyle(index:number, isActive:boolean){
    const selector = `#list > div:nth-child(${index})`; // nth-childは1番から
    const element = 
...
function setMemoElement(){
    ...
    // markdownで記述した本文(文字列を)HTMLにパースする
    (async ()=>{
        try{
...
function setEditMode(editMode:boolean){
    if(editMode){
        ...
        // 編集モード時はtextareaを表示し、プレビューを非表示にする
        ...
        // 表示モード時はtextareaを非表示し、プレビューを表示にする
